...
const hvLogger = winston.createLogger({
  level: "info",
  format: winston.format.combine(
    winston.format.timestamp({
      format: "YYYY-MM-DD HH:mm:ss"
...
const hvPollLogger = winston.createLogger({
  level: "info",
  format: winston.format.combine(
    winston.format.timestamp({
      format: "YYYY-MM-DD HH:mm:ss"
...
Error: ${info.error}`;
  ...
  transports: [
    new DailyRotateFile({
      ...
      datePattern: "YYYY-MM-DD",
      maxFiles: "14d"
      // Less retention for high-frequency logs
    }),
    new winston.transports.Console()
  ]
});
class TransactionPOSAudioService {
  static async createUpdateTransaction(data) {
    ...
    try {
      ...
      if (existing.length > 0) {
        ...
        await connection.query(
          `INSERT INTO DBA.TRANSACTIONPOSAUDIO(TRANSACT,STATUS,PHONENUMBER,DATEOUT,DATERETURN) VALUES (?, ?, ?, GETDATE(), ${data.Status === 1 ? "NULL" : "GETDATE()"})`,
   ...
   */
  static async autoOutNewTransactions(transact) {
    let connection;
    const pending = [];
    try {
      connection = await getConnection();
      ...
      const newTransactSql = `
        ...
        WHERE TPA.TRANSACT IS NULL
          ...
          ${scopeSql}
      `;
      const newTransacts = transact ? await connection.query(newTransactSql, [transact]) : await connection.query(newTransactSql);
      for (const row of newTransacts) {
        ...
        const details = detailRows.filter((d) => d.QUAN > 0).map((d) => ({
          PRODNUM: d.PRODNUM,
          QuantityOut: d.QUAN,
          QuantityReturn: 0
        }));
...
class HoangVanService {
  ...
  handleApiError(error, context) {
    if (axios.isAxiosError(error)) {
      ...
      if (status === 500)
        throw new Error(
          `Hoang Van server is experiencing issues (500). Details: ${typeof data === "object" ? JSON.stringify(data) : data || "None"}`
        );
...
class OrderService {
  static async deleteOrder(transact) {
...
electron.app.whenReady().then(() => {
  ...
  electron.ipcMain.handle("getReceiptTemplate", async () => {
    try {
      let templatePath = "";
      if (utils.is.dev) {
        templatePath = path.join(process.cwd(), "receipt.html");
      } else {
        templatePath = path.join(electron.app.getPath("exe"), "..", "receipt.html");
      }
