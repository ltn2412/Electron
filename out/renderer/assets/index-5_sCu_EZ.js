...
function PageMenu() {
  ...
  const executeUseHoangVanOrder = async () => {
    ...
    try {
      ...
      const finalHtml = templateToUse.replace(/{{ORDER_NO}}/g, hvOrderInfo.orderNo || "").replace(/{{CUSTOMER_NAME}}/g, hvOrderInfo.buyerName || "").replace(/{{EMAIL}}/g, hvOrderInfo.buyerEmail || "").replace(/{{VISIT_DATE}}/g, hvOrderInfo.visitDate || "").replace(
        /{{PURCHASE_DATE}}/g,
